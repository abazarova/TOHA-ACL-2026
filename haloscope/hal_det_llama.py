...
def main():
    ...
    if args.dataset_name == "tqa":
        ...
        dataset = get_dataset(LlamaTokenizer.from_pretrained(MODEL))
    else:
        raise ValueError("Invalid dataset name")

    if args.gene:
        tokenizer = LlamaTokenizer.from_pretrained(MODEL)
        model = LlamaForCausalLM.from_pretrained(MODEL, torch_dtype=torch.float16, device_map="auto").cuda()
        ...
        for i in trange(begin_index, end_index):
            if os.path.exists(f'./save_for_eval/{args.dataset_name}_hal_det/answers/most_likely_hal_det_{args.model_name}_{args.dataset_name}_answers_index_{i}.npy'):
                continue
          ...
          #  print('sample: ', i)
            ...
            np.save(f'./save_for_eval/{args.dataset_name}_hal_det/answers/' + info + f'hal_det_{args.model_name}_{args.dataset_name}_answers_index_{i}.npy', answers)
        ...
        for i in tqdm(range(length)):
            ...
            for anw in answers:
                ...
                with torch.no_grad():
                    ...
                    print(len(embed_generated))
        embed_generated = np.asarray(np.stack(embed_generated), dtype=np.float32)
        np.save(f'save_for_eval/{args.dataset_name}_hal_det/batch_generations_{args.model_name}_gene_embeddings_layer_wise.npy', embed_generated)
        ...
        # np.save(f'save_for_eval/{args.dataset_name}_hal_det/batch_generations_{args.model_name}_gene_embeddings_head_wise.npy', embed_generated_loc1)
        # np.save(f'save_for_eval/{args.dataset_name}_hal_det/batch_generations_{args.model_name}_embeddings_mlp_wise.npy',  embed_generated_loc2)
        ...
        if args.most_likely:
            if feat_loc == 3:
                embed_generated = np.load(f'save_for_eval/{args.dataset_name}_hal_det/batch_generations_{args.model_name}_gene_embeddings_layer_wise.npy',
                                  allow_pickle=True)
            elif feat_loc == 2:
                embed_generated = np.load(
                    f'save_for_eval/{args.dataset_name}_hal_det/batch_generations_{args.model_name}_gene_embeddings_mlp_wise.npy',
                    allow_pickle=True)
            else:
                embed_generated = np.load(
                    f'save_for_eval/{args.dataset_name}_hal_det/batch_generations_{args.model_name}_gene_embeddings_head_wise.npy',
                    allow_pickle=True)
            feat_indices_wild = []
            feat_indices_eval = []
